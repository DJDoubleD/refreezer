...
class Blowfish {
  ...
  */
  ...
  // P-array: Permutation boxes used in the Blowfish algorithm
  List<int> pArray = [
    0x243f6a88,
    0x85a308d3,
    0x13198a2e,
  ...
  // S-boxes: Substitution boxes used in the Blowfish algorithm
  List<List<int>> sBoxes = [
    [
      0xd1310ba6,
      0x98dfb5ac,
      0x2ffd72db,
  ...
  // using the Cipher-Block Chaining (CBC) mode of operation.
  //
  // Parameters:
  //   data: The data to decrypt as a Uint8List.
  //   iv: The initialization vector (IV) as a Uint8List.
  //
  // Returns:
  //   A Uint8List containing the decrypted data.
  Uint8List decrypt(Uint8List data, Uint8List iv) {
    if (data.length % BLOCK_SIZE != 0) {
      throw Exception('Invalid data length, must be a multiple of $BLOCK_SIZE');
    }

    Uint8List result = Uint8List(data.length);
    Uint32List lr = Uint32List(2);
    Uint8List block = Uint8List(BLOCK_SIZE);

    int prevLeft = ByteData.view(iv.buffer).getUint32(0, Endian.big);
    int prevRight = ByteData.view(iv.buffer).getUint32(4, Endian.big);
